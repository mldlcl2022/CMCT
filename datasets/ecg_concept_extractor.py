...
parser.add_argument('--sig_dir', type= str, required= True)
...
def main():
    ...
    for split in ['train','valid','test']:
        df = pd.read_csv(
            os.path.join(
                './splits', args.data_name, f'{split}.csv'
            )
        )
        
        if 'cpsc' in args.data_name: label_start_col = 7
        elif 'ptbxl' in args.data_name: label_start_col = 5
    ...
    for idx, filepath in enumerate(tqdm(df_total.filepath, desc= 'extract concept', unit= 'record')):
        '''load signal data'''
        if 'cpsc' in args.data_name: sig_path = os.path.join(args.sig_dir, args.data_name, filepath)
        elif 'ptbxl' in args.data_name: sig_path = os.path.join(args.sig_dir, args.data_name, filepath.replace('records500/',''))
    ...
    save_path = f'./{args.data_name}/concepts'
    ...
    print(f'Concept preparation complete. Results saved to: {save_path}')
